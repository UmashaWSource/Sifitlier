...
class DLPDetector:
    ...
    - Identity: SSN, NIC, passport numbers
    - Authentication: Passwords, PINs, OTPs, API keys
    ...
    def _init_patterns(self):
        ...
        NOTE: Each pattern uses inline (?i) where needed instead of global
        re.IGNORECASE, to prevent case-insensitive matching from causing
        false positives on uppercase-only patterns like SWIFT/BIC codes.
        """
        ...
        self.patterns: Dict[str, List[Tuple[str, str, SensitivityLevel, float]]] = {
            ...
            "credit_card": [
                ...
                # Generic 16-digit (only with card context to reduce false positives)
                (r'(?i)(?:card|credit|debit)[\s:#]*\b[0-9]{4}[-\s]?[0-9]{4}[-\s]?[0-9]{4}[-\s]?[0-9]{4}\b',
                 "Credit card number", SensitivityLevel.CRITICAL, 0.75),
            ],

            "bank_account": [
                # IBAN (International) - case sensitive, IBAN is uppercase
                (r'\b[A-Z]{2}[0-9]{2}[A-Z0-9]{4}[0-9]{7}([A-Z0-9]?){0,16}\b',
                 "IBAN", SensitivityLevel.HIGH, 0.95),
                # Account with context + "is" delimiter (various abbreviations)
                (r'(?i)(?:account|acct|a/c|acc)\s*(?:no|number|num|#)?[\s:#]*([0-9]{8,17})',
                 "Bank account number", SensitivityLevel.HIGH, 0.85),
                # "bank acc number is XXXX" or "account is XXXX"
                (r'(?i)(?:bank\s+)?(?:account|acct|a/c|acc)\s*(?:no|number|num|#)?\s+is\s+([0-9]{8,17})',
                 "Bank account number", SensitivityLevel.HIGH, 0.85),
                # "account: XXXX" or "account number: XXXX"
                (r'(?i)(?:bank\s+)?(?:account|acct|a/c)[\s:#]+[0-9]{8,17}',
                 "Bank account number", SensitivityLevel.HIGH, 0.85),
                # Routing number with context
                (r'(?i)(?:routing|rtg|aba)[\s:#]*([0-9]{9})',
                 "Bank routing number", SensitivityLevel.HIGH, 0.90),
                # SWIFT/BIC code - ONLY with context (prevents matching random words)
                (r'(?i)(?:swift|bic|bank\s*code)[\s:#]*[A-Z]{4}[A-Z]{2}[A-Z0-9]{2}([A-Z0-9]{3})?',
                 "SWIFT/BIC code", SensitivityLevel.HIGH, 0.85),
            ],

            "cvv": [
                # CVV with delimiter (: = is)
                (r'(?i)(?:cvv|cvc|cvv2|cvc2|security\s*code)[\s:]*([0-9]{3,4})',
                 "Card security code (CVV)", SensitivityLevel.CRITICAL, 0.95),
                # CVV/security code "is" XXXX
                (r'(?i)(?:cvv|cvc|cvv2|cvc2|security\s*code)\s+is\s+([0-9]{3,4})\b',
                 "Card security code (CVV)", SensitivityLevel.CRITICAL, 0.95),
            ],

            # ============== IDENTITY ==============
            "ssn": [
                # US Social Security Number (strict format with dashes/spaces)
            ...
            "nric": [
                # Singapore NRIC/FIN (case sensitive - starts with S/T/F/G/M uppercase)
            ...
            # Sri Lankan NIC - separate category
            "nic": [
                # Old format: 9 digits + V/X (e.g., 901234567V)
                (r'\b[0-9]{9}[VvXx]\b',
                 "Sri Lankan NIC (old format)", SensitivityLevel.CRITICAL, 0.95),
                # New format: 12 digits (e.g., 200012345678)
                (r'(?i)(?:nic|national\s*id|identity\s*card)[\s:#]*([0-9]{12})\b',
                 "Sri Lankan NIC (new format)", SensitivityLevel.CRITICAL, 0.90),
                # New format standalone - 12 digits starting with 19xx or 20xx
                (r'\b(?:19|20)[0-9]{10}\b',
                 "Sri Lankan NIC (new format)", SensitivityLevel.CRITICAL, 0.75),
            ],
            ...
            # ============== AUTHENTICATION ==============
            "password": [
                # Password with delimiter (: or =) — high confidence
                (r'(?i)password[\s]*[:=]+[\s]*\S+',
                 "Password", SensitivityLevel.CRITICAL, 0.95),
                # Password "is" + value that looks like a real password (has digit or special char)
                (r'(?i)password\s+is\s+(\S*[0-9!@#$%^&*_]+\S*)',
                 "Password", SensitivityLevel.CRITICAL, 0.90),
                # Password "is" + value (lower confidence, could be metaphorical)
                (r'(?i)password\s+is\s+([A-Za-z0-9!@#$%^&*_]{6,})',
                 "Password", SensitivityLevel.CRITICAL, 0.70),
                # pwd/passwd with delimiters
                (r'(?i)(?:pwd|passwd)[\s]*[:=]+[\s]*\S+',
                 "Password", SensitivityLevel.CRITICAL, 0.90),
                (r'(?i)(?:pwd|passwd)\s+is\s+\S+',
                 "Password", SensitivityLevel.CRITICAL, 0.85),
                # "pass is X" for casual SMS (but require 6+ chars to avoid "pass is ok")
                (r'(?i)\bpass\s+is\s+\S{6,}',
                 "Password", SensitivityLevel.CRITICAL, 0.75),
                # "my pass is X" / "ur pass is X" — casual SMS style
                (r'(?i)(?:my|ur|your)\s+pass\s+is\s+\S+',
                 "Password", SensitivityLevel.CRITICAL, 0.80),
                # wifi password pattern
                (r'(?i)(?:wifi|wi-fi)\s+password\s+is\s+\S+',
                 "Password", SensitivityLevel.CRITICAL, 0.90),
            ],

            "pin": [
                # PIN with delimiter (: = is)
                (r'(?i)(?:pin|pin\s*code|pin\s*number)[\s]*[:=]+[\s]*[0-9]{4,6}',
                 "PIN code", SensitivityLevel.CRITICAL, 0.95),
                # PIN is XXXX
                (r'(?i)(?:pin|pin\s*code|pin\s*number)\s+is\s+[0-9]{4,6}\b',
                 "PIN code", SensitivityLevel.CRITICAL, 0.95),
            ],
            ...
            "otp": [
                # OTP with delimiter
                (r'(?i)(?:otp|one[\s-]?time[\s-]?(?:password|code|pin)|verification\s*code|verify\s*code)[\s]*[:=]+[\s]*[0-9]{4,8}',
                 "OTP/Verification code", SensitivityLevel.HIGH, 0.95),
                # OTP is XXXX
                (r'(?i)(?:otp|one[\s-]?time[\s-]?(?:password|code|pin)|verification\s*code)\s+is\s+[0-9]{4,8}\b',
                 "OTP/Verification code", SensitivityLevel.HIGH, 0.95),
                # Common OTP SMS pattern: "Your OTP is 123456" or "OTP: 123456"
                (r'(?i)(?:your\s+)?(?:otp|code)\s+(?:is\s+)?[0-9]{4,8}\b',
                 "OTP/Verification code", SensitivityLevel.HIGH, 0.80),
                # OTP with words in between: "OTP for banking is 123456"
                (r'(?i)\bOTP\s+(?:\w+\s+){0,4}(?:is\s+)?[0-9]{4,8}\b',
                 "OTP/Verification code", SensitivityLevel.HIGH, 0.75),
            ],

            "api_key": [
                # API keys with various delimiters
                ...
                (r'(?i)access[_-]?token[\s:=]+[A-Za-z0-9_\-]{16,}',
                 "Access Token", SensitivityLevel.CRITICAL, 0.95),
                # Keys starting with common prefixes (sk-, pk-, etc.)
                (r'(?i)(?:api[_-]?key|key|token)[\s:=]+(?:sk|pk|rk|ak)-[A-Za-z0-9_\-]{16,}',
                 "API Key", SensitivityLevel.CRITICAL, 0.95),
                # Bearer tokens
                (r'(?i)bearer[\s]+[A-Za-z0-9_\-\.]{20,}',
                 "Bearer Token", SensitivityLevel.CRITICAL, 0.90),
                # AWS keys (case sensitive — always starts with AKIA)
                (r'AKIA[0-9A-Z]{16}',
            ...
            "phone": [
                ...
                (r'\+94[-\s]?[0-9]{2}[-\s]?[0-9]{3}[-\s]?[0-9]{4}',
                 "Phone number (Sri Lankan)", SensitivityLevel.MEDIUM, 0.90),
                # Sri Lankan local (07XXXXXXXX)
                (r'\b07[0-9]{8}\b',
                 "Phone number (Sri Lankan local)", SensitivityLevel.MEDIUM, 0.85),
                # International format (general)
                (r'\+[1-9][0-9]{0,2}[-\s]?[0-9]{2,4}[-\s]?[0-9]{3,4}[-\s]?[0-9]{3,4}',
                 "Phone number (international)", SensitivityLevel.MEDIUM, 0.85),
                # US format
                (r'\b\(?[0-9]{3}\)?[-.\s]?[0-9]{3}[-.\s]?[0-9]{4}\b',
                 "Phone number (US)", SensitivityLevel.MEDIUM, 0.80),
                # Phone with context keyword
                (r'(?i)(?:phone|mobile|cell|tel|call\s+me)[\s:#]*([0-9\-\s]{10,})',
            ...
            "dob": [
                # Date of birth with delimiter (: = is)
                (r'(?i)(?:dob|date\s*of\s*birth|born|birthday)[\s:]+[0-9]{1,2}[/\-][0-9]{1,2}[/\-][0-9]{2,4}',
                 "Date of birth", SensitivityLevel.MEDIUM, 0.90),
                # "date of birth is XX/XX/XXXX"
                (r'(?i)(?:dob|date\s*of\s*birth|born|birthday)\s+is\s+[0-9]{1,2}[/\-][0-9]{1,2}[/\-][0-9]{2,4}',
                 "Date of birth", SensitivityLevel.MEDIUM, 0.90),
            ],
    ...
    def analyze(self, text: str) -> Dict[str, Any]:
        ...
        for category, patterns in self.patterns.items():
            for pattern, description, sensitivity, confidence in patterns:
                # NO global re.IGNORECASE — each pattern uses (?i) inline if needed
    ...
    def redact_text(self, text: str) -> str:
        """
        Return a copy of the text with all sensitive data replaced by masks.
        Useful for storing redacted versions instead of raw messages.
        """
        result = self.analyze(text)
        if not result['has_sensitive_data']:
            return text

        # Sort matches by position (end first) so replacements don't shift offsets
        sorted_matches = sorted(result['matches'], key=lambda m: m['position']['start'], reverse=True)

        redacted = text
        for match in sorted_matches:
            start = match['position']['start']
            end = match['position']['end']
            redacted = redacted[:start] + match['masked_text'] + redacted[end:]
    ...
    def _mask_text(self, text: str, category: str) -> str:
        ...
        elif category in ["password", "pin", "api_key", "cvv", "otp"]:
            # Fully mask
...
def test_dlp_detector():
    ...
    test_cases = [
        ...
        ("PIN is 1234", SensitivityLevel.CRITICAL),
        ("My NRIC is S1234567D", SensitivityLevel.CRITICAL),
        ("My NIC is 901234567V", SensitivityLevel.CRITICAL),
        ("ur pass is qwerty", SensitivityLevel.CRITICAL),
        ("wifi password is HomeNetwork99", SensitivityLevel.CRITICAL),

        # High
        ("Account number: 12345678901234", SensitivityLevel.HIGH),
        ("IBAN: GB82WEST12345698765432", SensitivityLevel.HIGH),
        ("OTP is 456789", SensitivityLevel.HIGH),
        ("Your verification code is 1234", SensitivityLevel.HIGH),

        # Medium
        ("Call me at +94 77 123 4567", SensitivityLevel.MEDIUM),
        ("My number is 0771234567", SensitivityLevel.MEDIUM),
        ("DOB: 01/15/1990", SensitivityLevel.MEDIUM),

        # Low
        ("Email me at john@example.com", SensitivityLevel.LOW),

        # None - should NOT detect
        ("Hey, how are you doing today?", SensitivityLevel.NONE),
        ("Meeting at 3pm tomorrow", SensitivityLevel.NONE),
        ("The password to success is hard work", SensitivityLevel.NONE),
        ("Room 1234 at the hotel", SensitivityLevel.NONE),
        ("Your order number is 123456", SensitivityLevel.NONE),
        ("Call me when you're free", SensitivityLevel.NONE),
    ]
    ...
    for text, expected_level in test_cases:
        ...
        if expected_level == SensitivityLevel.NONE:
            # For NONE cases, we expect no detection
            if not result["has_sensitive_data"]:
                status = "PASS"
                passed += 1
            else:
            ...
            # For positive cases, check sensitivity matches or exceeds
